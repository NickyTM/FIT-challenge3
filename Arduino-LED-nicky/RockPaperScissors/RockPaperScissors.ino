...
float flexMean;
float touchMean;
float flexThreshold = 600; //if flex is bent
float touchThreshold = 100; //if cap is touch
int pose = 0;
...
void setup() {
  ...
  establishContact(); //contact with processing through serial port
  // game_started = true;
}
...
//  Serial.println("Ontvang ik dit?");
//  Serial.println(str);
  
  calculateWinner(pose, 1);
//  debugValues();
...
void calculateWinner(int myPose, int opponentInput) {
...
//
//  // output
//  pinMode(8, OUTPUT); // Vibration motor 1 - outside pink
//  pinMode(9, OUTPUT); // Vibration motor 2 - outside pointer
//  pinMode(10, OUTPUT);// Vibration motor 3 - inside hand
//  pinMode(11, OUTPUT);// LED red
//  pinMode(12, OUTPUT);// LED green
//
//  cs_4_2.set_CS_AutocaL_Millis(0xFFFFFFFF); // Calibration capsensor
...
////  debugValues();
//
////    String serverStr = Serial.readString();
////    Serial.println("The server has sent: " + serverStr);
//
//  calculateWinner(pose, 1);
//}
//
...
//    winnerAction();
////    digitalWrite(12, HIGH);
//  } else if (myPose == 3 && opponentInput == 1 ) {
//    // Scissors lost from rock
//    digitalWrite(8, LOW);
//    digitalWrite(9, HIGH);
//    digitalWrite(10,LOW);
